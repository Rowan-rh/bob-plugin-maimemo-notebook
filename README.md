...
配置 MiniMax CN API Token 后，插件还可以自动分析输入的英文内容，提取有学习价值的单词、固定搭配和短语，并查询它们是否收录在墨墨词库中。已收录词条会直接加入云词本；未收录词条会由 AI 翻译并整理成带音标、词性、释义和记忆联想的自定义内容。
...
![Find your own notepad id](https://webp.chriscurry.cc/uZNenI.jpg)
...
1. 在 Bob 插件配置中填写「MiniMax CN API Token」，可选模型默认为 MiniMax-M3。
2. 输入英文单词、短语或句子后，插件会自动整理重点单词、固定搭配和短语，查询墨墨词库并把结果写入云词本。
3. 墨墨已收录的词条以原文加入；未收录的词条会生成自定义内容，包含音标、词性、中文释义、词形与读音联想、代码场景、概念对比、画面比喻和谐音助记。词源在模型有把握时才会附上。
4. 如果同时启用了「添加例句到生词」，原句及其 AI 译文还会作为例句添加到墨墨已收录的单词。

墨墨开放 API 没有新增自定义词汇的接口，因此未收录词/短语会以多行文本保存到云词本；它们不能作为墨墨词库中的正式词条加入每日学习计划。模型生成的代码路径和类名是帮助记忆的虚构类比，不对应真实代码库。
